...
class ObAttnAEEnc(nn.Module):
    ...
    def __init__(self,
        ...
        self.query = nn.Linear(input_shape, self.args.attn_embed_dim)#attn_embed_dim: 16   权重是来自于query和key的东西
        self.key = nn.Linear(input_shape, self.args.attn_embed_dim)
        self.value = nn.Linear(input_shape, self.args.encoder_hidden_dim)#encoder_hidden_dim: 32  输出其实是与value的加权和
