...
class MASIAAgent(nn.Module):
    ...
    def target_transform(self, inputs, encoder_hidden_state):
        ...
        if "vae" in self.args.state_encoder:
            ...
            z, encoder_h = self.target_encoder.encode(inputs, encoder_hidden_state)#target_encode应该是论文部分右边那块的encoder EMAupdate
        else:
            raise ValueError("Unknown encoder!!!")

        # do projection
        target_projected = self.target_projection(z)#将Z过一个网络投射到另外更低维的一个空间目的是学习数据的紧凑表示，以便在潜在空间中保持数据的重要特征
    ...
    def _build_inputs(self, inputs):
        ...
        base_inputs = inputs[:, :self.raw_input_shape] # input_shape = [bs*n_agents, ob_embed_dim]
